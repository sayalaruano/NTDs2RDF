...
for (i in df$entrezgene_id){
  ...
  if(is.null(wps)){
    ...
    temp_df <- data.frame(entrezgene_id = wps@result[1:30,]$geneID, 
                          wikipathID = wps@result[1:30,]$ID, 
                          description = wps@result[1:30,]$Description)
...
for (i in df$entrezgene_id){
  ...
  if(is.null(react)){
    ...
    # Select the top 20 diseases 
    temp_df <- data.frame(hgnc_symbol = react@result[1:30,]$geneID, 
                          reactpathID = react@result[1:30,]$ID, 
                          description = react@result[1:30,]$Description)
